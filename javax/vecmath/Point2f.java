...
public class Point2f extends Tuple2f implements Serializable
{
    ...
    public final float distanceSquared(final Point2f p1) {
        final double dx = super.x - ((Tuple2f)p1).x;
        final double dy = super.y - ((Tuple2f)p1).y;
    ...
    public final float distanceL1(final Point2f p1) {
        return Math.abs(super.x - ((Tuple2f)p1).x) + Math.abs(super.y - ((Tuple2f)p1).y);
    }
    
    public final float distanceLinf(final Point2f p1) {
        return Math.max(Math.abs(super.x - ((Tuple2f)p1).x), Math.abs(super.y - ((Tuple2f)p1).y));
    }
}

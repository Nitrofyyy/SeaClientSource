...
public enum FunctionType
{
    ...
    private FunctionType(final String name2, final int ordinal, final ExpressionType expressionType, final String name, final ExpressionType[] parameterTypes) {
        ...
        this.parameters = new IParameters() {
            @Override
            public ExpressionType[] getParameterTypes(final IExpression[] var1) {
    ...
    public float evalFloat(final IExpression[] args) {
        switch (this) {
            ...
            case ATAN2: {
                return MathHelper.func_181159_b(evalFloat(args, 0), evalFloat(args, 1));
            }
            case TORAD: {
    ...
    public static FunctionType parse(final String str) {
        FunctionType[] values;
        for (int length = (values = FunctionType.VALUES).length, i = 0; i < length; ++i) {

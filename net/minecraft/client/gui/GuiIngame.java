...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(final float partialTicks) {
        ...
        if (!(this.mc.currentScreen instanceof HUDConfigScreen)) {
            Client.INSTANCE.hudManager.renderMod();
        }
        GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
    ...
    public void renderHorseJumpBar(final ScaledResolution scaledRes, final int x) {
        ...
        Gui.drawTexturedModalRect(x, k, 0, 84, 182, 5);
        if (j > 0) {
            Gui.drawTexturedModalRect(x, k, 0, 89, j, 5);
        }
        this.mc.mcProfiler.endSection();
    }
    ...
    private void renderScoreboard(final ScoreObjective objective, final ScaledResolution scaledRes) {
        ...
        for (final Score score : collection) {
            ...
            final String s = String.valueOf(String.valueOf(ScorePlayerTeam.formatPlayerName(scoreplayerteam, score.getPlayerName()))) + ": " + EnumChatFormatting.RED + score.getScorePoints();
            ...
            if (entity == null) {
                ...
                while (j9 > 0) {
                    ...
                    for (int i9 = 0; i9 < l6; ++i9) {
                        ...
                        Gui.drawTexturedModalRect(l7, i8, 52 + k9 * 9, 9, 9, 9);
                        if (i9 * 2 + 1 + k8 < i7) {
                            Gui.drawTexturedModalRect(l7, i8, 88, 9, 9, 9);
                        }
                        if (i9 * 2 + 1 + k8 == i7) {
                            Gui.drawTexturedModalRect(l7, i8, 97, 9, 9, 9);
                        }
                    }
                    i8 -= 10;
                    k8 += 20;
    ...
    private void renderBossHealth() {
        if (BossStatus.bossName != null && BossStatus.statusBarTime > 0) {
            ...
            final int i2 = 12;
            Gui.drawTexturedModalRect(k, 12, 0, 74, 182, 5);
            Gui.drawTexturedModalRect(k, 12, 0, 74, 182, 5);
            if (l > 0) {
                Gui.drawTexturedModalRect(k, 12, 0, 79, l, 5);
            }
